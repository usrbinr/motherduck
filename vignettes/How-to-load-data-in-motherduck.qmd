...
The `connect_to_motherduck()` function will take your access token that is your environment file[^1], install and load the extensions and then finally connect to your motherduck instance.

[^1]: Use `usethis::edit_r_environ()` to save your access token to a variable name
...
For csv files we can leverage the existing duckdb function `duckdb::read_csv_duckdb()` to directly read the a csv file or a series of csv files[^3] into your duckdb or motherduck database
...
You can view the default configuration tables with the motherduck::config\_\* family of tables
...
openxlsx::write.xlsx(dplyr::starwars,"starwars.xlsx") #<1>
